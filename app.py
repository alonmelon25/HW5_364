...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/proaaronHW5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    priority = StringField("Enter the new priority for the selected item: ", validators = [Required()])
    submit = SubmitField("Submit")
...
def delete(lst):
    ...
    t = TodoList.query.filter_by(id=lst).first()
    db.session.delete(t)
    db.session.commit()
    flash("Successfully deleted: {}".format(t.title))
    return redirect(url_for('all_lists'))
